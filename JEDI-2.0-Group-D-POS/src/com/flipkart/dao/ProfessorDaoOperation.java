...
public class ProfessorDaoOperation implements ProfessorDaoInterface {
	...
	public List<EnrolledStudent> getEnrolledStudents(String courseId) {
		...
		try {
			...
			while(results.next())
			{
				System.out.println("\n" + results.getString("courseCode") + results.getString("courseName")+ results.getString("studentId"));
	...
	public Boolean addGrade(String studentId,String courseCode,String grade) {
		...
		try {
			...
			if(row==1) {
				statement = connection.prepareStatement(SQLQueriesConstant.SET_REPORT_STATUS);
